...
| `@deepseek-ai/dsh-tool-subagent` | `list_subagent_models`、`request_subagent_model_routes`、`subagent` | `ctx.tools`、`ctx.subagents`、`ctx.systemPrompt`、`用于模型发现和所选路由校验的 ctx.llm`、`用于路由批准请求的 ctx.userQuestions（执行时）` | `tool/call`、`tool/result`、`child session events through the chosen provider` | `subagent`、`subagent_fork` | 注册的委派工具名称取决于加载时 `toolName` 配置（默认为 `subagent`）；上述默认 schema 关闭模型选择，而发现 schema 则展示为已启用 Session 中可用的固定配套工具。Web preset 会在每个新顶层 Session 创建时读取插件页偏好，并为其子 Session 保留该决定；`subagent_fork` 始终使用固定路由。每个实例通过 `modelSelectionSettings`、`backgroundMode` 与 `enableRunInBackground` 独立控制是否读取模型选择设置及其后台行为。 `request_subagent_model_routes` 注册在 settings 控制实例的顶层 Session 中，只有在用户批准确切的结果列表后才会改变该 Session 的允许列表。 |
...
请用户更改本 session 可为 subagent 和 teammate 选择的子级 LLM 路由。用户会看到确切的结果允许列表，必须批准才会生效；否则不做任何更改。路由写作 provider/model，与 list_subagent_models 报告的形式一致。仅在用户需要当前未允许的路由时使用。
...
来源：[`packages/subagent/tool-subagent/src/model-selection-approval.ts`](../packages/subagent/tool-subagent/src/model-selection-approval.ts)
...
注册的委派工具名称取决于加载时 `toolName` 配置（默认为 `subagent`）；上述默认 schema 关闭模型选择，而发现 schema 则展示为已启用 Session 中可用的固定配套工具。Web preset 会在每个新顶层 Session 创建时读取插件页偏好，并为其子 Session 保留该决定；`subagent_fork` 始终使用固定路由。每个实例通过 `modelSelectionSettings`、`backgroundMode` 与 `enableRunInBackground` 独立控制是否读取模型选择设置及其后台行为。 `request_subagent_model_routes` 注册在 settings 控制实例的顶层 Session 中，只有在用户批准确切的结果列表后才会改变该 Session 的允许列表。
...
以同一个名字为一名已结束的 teammate 换一条新的 LLM 路由。旧成员被取代：它保留自己的 id、历史、路由与结果，而它的名字与 roster 槽位移交给替代者。旧成员持有的每一项进行中任务都会移交给替代者，并在 transferredTasks 中报告；已完成的任务保留完成它的那位 owner。之后按该名字的指派都会落到替代者身上。请向替代者交代它接手的工作：它不携带被取代成员的任何记忆。用它来挽回一名在错误路由上启动的 teammate，而不必再烧掉一个名字。目标不能处于运行中，请先中断它。只有 Team Lead 可以调用此工具。
...
```json
{
  ...
  "properties": {
    "name": {
      "type": "string",
      "description": "Name of the teammate to replace. The replacement answers to the same name."
    },
    "prompt": {
      "type": "string",
      "description": "Complete initial task for the replacement. It starts with no memory of the superseded member."
    },
    "provider": {
      "type": "string",
