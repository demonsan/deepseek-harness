...
以同一个名字为一名已结束的 teammate 换一条新的 LLM 路由。旧成员被取代：它保留自己的 id、历史、路由与结果，而它的名字与 roster 槽位移交给替代者。用它来挽回一名在错误路由上启动的 teammate，而不必再烧掉一个名字。目标不能处于运行中，请先中断它。只有 Team Lead 可以调用此工具。
