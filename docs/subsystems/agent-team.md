...
Every member starts in `provisioning` and reaches exactly one terminal roster phase, `active` or `failed`. Roster `running`/`inactive` status is derived separately and never rewrites this record. A settled member admits exactly one further edge: supersession, which sets `supersededBy` and rewrites nothing else, so the row keeps the outcome it reached while its name and roster slot pass to the replacement.
